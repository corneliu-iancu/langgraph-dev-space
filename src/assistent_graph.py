...
from src.utils import get_message_text, load_chat_model, format_docs
from langchain_core.messages import BaseMessage, AIMessage, ToolMessage, HumanMessage
from src.retriever import make_retriever
from pydantic import BaseModel
from datetime import datetime, timezone
from typing import cast
from src.state import State, InputState
...
async def generate_routing_query(state: State, *, config: RunnableConfig):
    """Generate a routing query based on the current state.

    Args:
        state (State): Current state containing messages.
        config (RunnableConfig | None, optional): Configuration for query generation.

    Returns:
        dict[str, list[str]]: Dictionary with 'queries' key containing generated queries.
    """
